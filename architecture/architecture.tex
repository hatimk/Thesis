...
\section{Illustration}
	...
	\subsection{A textual dialogue system: CFAsT}
        ...
        This implementation is aimed to provide a first proof of concept of the methodology introduced here. However, signal\_ETC = \textit{CommitCond} = \textit{EndTurnCond} (the Scheduler never takes commit decisions by itself). It gives a graphical representation of the functioning of the approach introduced here. It also illustrates how the latter can be used to perform auto-completion: as the user gives more information, the set of possible requests shrinks until converging to one possibility. These possibilities can be proposed on the fly, as the user is typing.
