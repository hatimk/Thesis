	...
	\subsection{The service: personal agenda assistant}
		...
		\subsubsection{Natural Language Understanding}
			...
			\subsection{Simulator instanciation}
				...
				\begin{table}[htp]
						...
						\centerline{
							\begin{tabular}{|l|l|l|}
								...
								BF & 0.2 & Same \\
								$\sigma_{conf}$ & 1 & Empirical (variable and difficult to estimate) \\
				...
				The aim of the US is to make the biggest number possible of events with the highest priority values taken from \textit{InitList} and \textit{ToAddList} fit in the agenda. To do so, the Intent Manager executes the \textit{run} function (taking the action stack (AS) as an argument) depicted in Algorithm \ref{algo:intent} (an instance of the abstract function depicted in Algorithm \ref{algo:abstractim}) where
